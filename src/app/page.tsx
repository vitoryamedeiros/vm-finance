...
import { Send, ArrowUpRight, ArrowDownRight, CreditCard, Wallet } from "lucide-react";
import { supabase as sb } from "@/lib/supabase/client"; // Next RC Server components can use standard supabase JS if configured carefully
import { processSmartInput } from "@/app/actions";
import { generateInsights } from "@/lib/insights";

export default async function Home() {
  
  // Real data fetching directly in Server Component
  // In a real app we would use fetch with caching or supabase-ssr. For MVP, this direct call is fine.
    ...
    <div className="min-h-screen bg-background flex flex-col font-sans selection:bg-primary selection:text-white">
      ...
      <main className="flex-1 w-full max-w-[1400px] mx-auto px-4 md:px-8 py-8 md:py-12 space-y-12">
        ...
        <section className="flex flex-col gap-6 max-w-4xl">
          ...
          <form action={async (fd) => { "use server"; await processSmartInput(fd); }} className="relative mt-4 flex flex-col w-full max-w-3xl group">
             <div className="relative flex items-center w-full">
               <Input 
                  name="smartInput"
                  placeholder="Ex: Gastei 150 de gasolina no cartão nubank" 
                  className="h-20 text-xl pl-6 pr-24 rounded-none"
                  autoComplete="off"
               />
               <Button type="submit" size="icon" variant="brutal" className="absolute right-3 top-3 h-14 w-14 shrink-0 p-0">
